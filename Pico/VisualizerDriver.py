...
class HT16K33:
    ...
    def __init__(self, address=0x70, auto_write=True, brightness=1.0, i2c_obj: I2C | SoftI2C | None = None):
        if i2c_obj is None:
            self.i2c_device = I2C(0, freq=200000)
        else:
...
class VisualizerDriver:
    def __init__(self, address=0x70, i2c_obj: I2C | SoftI2C | None = None):
    ...
    def set_bar(self, bar_num: int, level: int):
        ...
        # bars 8-9 are complex because they share commons
        elif bar_num >= 8 and bar_num < 10:
            new_bar_num = (bar_num - 8) * 2
            # clear the bar for the 2 commons
            for i in range(new_bar_num, new_bar_num + 2):
                for j in range(10, 16):
                    self._ht.set_pixel(i, j, LOW)

            # if less than 6 then we only need one common
            if level > 6:
                # set for the first common
                for j in range(10, 16):
                    self._ht.set_pixel(new_bar_num, j, HIGH)
                # set for second common
